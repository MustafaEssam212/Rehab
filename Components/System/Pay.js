...
const Pay = () => {
    ...
    const getData = async (pageNum, search = false) => {
        ...
        const res = await fetch(`/api/ERP?method=get-pays&page=${pageNum}&search=${search}&date=${date}&related=${related}`);
    ...
    const handleAddNewPay = async () => {
        ...
        if(!newPay.amount ||!newPay.date || !newPay.name){
            ...
            const res = await fetch(`/api/ERP?method=add-new-pay`, {
                method: 'POST',
    ...
    const handleDeletePay = async () => {
        ...
        if(!deletePayNumber){
            ...
            const res = await fetch(`/api/ERP?method=delete-pay&number=${deletePayNumber}`, {
                method: 'DELETE',
                headers: {
                    'Content-Type': 'application/json'
                }
    ...
    return(
        <div className="pay-component-erp">
            ...
            <div className="pay-bar">
                <button onClick={()=> setOpenDeletePayMenu(!openDeletePayMenu)}>مسح دفع</button>
                <button onClick={()=> setOpenAddNewPayMenu(!openAddNewPayMenu)}>اضافة دفع جديد</button>
                ...
                openAddNewPayMenu && <div ref={addNewPayDivRef} className="add-new-pay-bar">
                    ...
                    <div className="right-price-bar-erp">
                            ...
                            <div>
                                <input value={newPay.name} placeholder="اسم الدفع" onChange={(s)=> setNewPay({...newPay, name: s.target.value})} type="text"></input>
                                <p>اسم الدفع</p>
                            </div>
                ...
                openDeletePayMenu && <div ref={deletePayDivRef} className="delete-pay-bar">
                    ...
                    <div className="right-price-bar-erp">
                            <div>
                                <input placeholder="رقم الدفع" onChange={(s)=> setDeletePayNumber(s.target.value)} type="number"></input>
                                <p>رقم الدفع</p>
                            </div>
                    </div>
                </div>
                                        ...
                                        pays.map((e, key) => {
                                            return(
                                                <div key={key} className="pay-div">
                                                    <p>{e.serial} <span>:رقم الدفع</span></p>
