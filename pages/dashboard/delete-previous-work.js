...
export async function getServerSideProps(context) {
    ...
    if (!session || session.user.role !== 'admin' || !session.user.permissions.includes('مسح سابقة اعمالنا')) {
...
const RemoveBlog = () => {
    ...
    const getData = async () => {
        const res = await fetch(`/api/getData?method=get-previous-works-names`);
        const dataOfResponse = await res.json();
        if(res.status === 200){
            setBlogsNames(dataOfResponse.previousWorks);
        }else{
    ...
    const handleDelete = async () => {
        ...
        if(!blog){
            ...
            const res = await fetch(`/api/editData?method=delete-previous-work&previousWork=${blog}`, {
                method: 'DELETE',
                headers: {
    ...
    return(
        <div className="remove-blog-page system-page">
            <HeadSystem name={`مسح سابقة اعمالنا`} />
