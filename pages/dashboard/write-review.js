...
export async function getServerSideProps(context) {
    ...
    if (!session || session.user.role !== 'admin' || !session.user.permissions.includes('كتابة اراء عملائنا')) {
...
const WritePerviousWork = () => {
    ...
    const handleAddKeyword = () => {
      ...
      const handleSubmit = async () => {
       ...
       const res = await fetch(`/api/upload?method=add-review`, {
        method: 'POST',
        body: formData
        });
    ...
    return(
        <div className="write-previous-work-page system-page">
            <HeadSystem name={`كتابة اراء عملائنا`} />
